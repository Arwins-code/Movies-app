...
class TrendingTvShowsSource @Inject constructor(private val api: TvShowTMDBApi) :
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TvShow> {
        return try {
            ...
            val trendingTvShows = api.getTrendingTvShows(nextPage)
            LoadResult.Page(
                data = trendingTvShows.results,
                prevKey = if (nextPage == 1) null else nextPage - 1,
                nextKey = if (trendingTvShows.results.isEmpty()) null else trendingTvShows.page + 1
            )
